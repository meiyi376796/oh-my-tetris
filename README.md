...
`Oh My Tetris` 是一个基于 `pygame` 的本地单机项目，配一套 Game Boy 荧光绿 LCD 风格的扁平界面。
...
- 七袋随机（`7-bag`）发牌，`SRS` 旋转与踢墙，下一块（`NEXT`）预览与影子落点。
- `DAS` 16 帧 / `ARR` 6 帧、软降每格得 1 分、按锁定高度 10~18 帧的入场延迟、17~20 帧从中心向两侧的消行动画，时序对齐 NES 经典版。
- 消除 `10` 行升一级；分数 = 消行基础分 × `(等级 + 1)`（单消 `40`、双消 `100`、三消 `300`、四消 `1200`）。
- 局内暂停/重开，窗口失焦自动暂停。
- Game Boy 式布局：顶部标题、左侧棋盘、右侧信息栏（分数 / 行数 / 等级 / 下一块预览）、底部按键说明。
- 掌机细节：棋盘旁的红色电源指示灯（对局常亮、暂停闪烁、熄灭待机）、画布颗粒纹理、屏幕内阴影、浮雕方块与标题压印。
...
下面的示例默认按 `python3` 来写；如果你的系统里没有这个命令，也可以改用 `python`。
...
- `TetrisUI` 负责界面：标题、右侧信息栏（含下一块预览）、棋盘、按键提示和状态遮罩。
